 ...
 */
public class MydaishouhuoModel extends DVolleyModel {

    private DResponseService consultListResponse;

    public MydaishouhuoModel(Context context) {
    ...
    private class ConsultListResponse extends DResponseService {
        ...
        protected void myOnResponse(DCallResult callResult) throws Exception {
                ...
                LogUtil.i("代收货信息请求数据","callResult="+callResult);
            String reult = callResult.getResponse().toString();
            LogUtil.i("代收信息请求数据","reult="+reult);
            JSONArray contentArray = callResult.getContentArray();
            LogUtil.i("代收信息请求数据","contentArray="+contentArray.length()+"contentArray="+contentArray.toString());
            try {
                ...
                for (int i = 0; i < contentArray.length(); i++) {
                    ...
                    LogUtil.i("代收货信息请求数据" + i, dingdanEntity.toString());
                    list.add(dingdanEntity);
                }
                LogUtil.i("代收货信息请求数据", list.toArray());
                returnBean.setObject(list);
            }catch (Exception e){
                e.printStackTrace();
            }
            returnBean.setType(DVolleyConstans.METHOD_GEREN_YIDAOHUO);
